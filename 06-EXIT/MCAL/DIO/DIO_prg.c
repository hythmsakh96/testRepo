...
u8 MDIO_u8GetPintVal(u8 A_u8PortNo,u8 A_u8PinNo)
{
	u8 returnVlaue;
	switch(A_u8PortNo)
	{
	case DIO_PORTA:
		returnVlaue= GET_BIT(PINA,A_u8PinNo);
		break;
	case DIO_PORTB:
		returnVlaue =GET_BIT(PINB,A_u8PinNo);
		break;
	case DIO_PORTC:
		returnVlaue =GET_BIT(PINC,A_u8PinNo);
		break;
	case DIO_PORTD:

		returnVlaue= GET_BIT(PIND,A_u8PinNo);
		break;
	}
	return returnVlaue;
}
